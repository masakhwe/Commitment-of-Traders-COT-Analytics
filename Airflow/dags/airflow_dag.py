...
with DAG(
    ...
    pandas_gcp_connection_task = PythonOperator(
        task_id = "pandas_gcp_connection_task",
        python_callable=pipeline.pandas_gcp_connection,
    ...
    write_df_to_gcs_task = PythonOperator(
        task_id = "write_df_to_gcs_task",
        python_callable = pipeline.write_df_to_gcs_in_parquet
    )
    ...
    download_dataset_task >> unzip_dataset_task >> local_to_gcs_task >> delete_processed_files_task >>pandas_gcp_connection_task >> organize_columns_task >> get_latest_data_task >> clean_columns_task >> write_df_to_gcs_task >> write_df_to_bigquery_task